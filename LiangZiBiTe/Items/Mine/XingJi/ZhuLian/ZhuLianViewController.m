...
@interface ZhuLianViewController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation ZhuLianViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.tableView.backgroundColor = LightHexColcor;
    
    [self setNavgiationBarTitle:@"推广主链"];
...
- (void)loadData{
    ...
    [App_HttpsRequestTool tuiguangPersonWithtype:@"1" Success:^(id responseObject) {
        [self endRefresh];
        ...
        if ([response isSuccess]) {
            ...
            self.titleLabel.text = [NSString stringWithFormat:@"主链业绩：%@",response.data.count];
